...
const AttendanceReports: React.FC<AttendanceReportsProps> = ({
  ...
  const getStudentAttendanceRate = (studentId: number, courseId: string) => {
    const studentRecords = attendanceRecords.filter(r => 
      r.studentId === studentId && r.course === courseId
    );
    if (studentRecords.length === 0) return 0;
    
    const presentRecords = studentRecords.filter(r => 
          ...
          <CardContent>
            <div className="space-y-4">
              {students.map(student => {
                const attendanceRate = getStudentAttendanceRate(student.id, course.id);
                const studentRecords = attendanceRecords.filter(r => 
                  r.studentId === student.id && r.course === course.id
                );
                  ...
                  <div key={student.id} className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <div className="flex items-center space-x-6">
                      <div className="text-center">
                        ...
                        <p className="font-bold">{studentRecords.length}</p>
                      </div>
                      <div className="text-center">
